...
/**
 * Rest model class for Ultimate Users.
 *
 * @package		Joomla.Site
 * @subpackage	com_users
 * @since		1.5
 */
class UuModelReset extends JModelForm
{
	/**
	...
	{
		// Get the form.
		$form = $this->loadForm('com_uu.reset_request', 'reset_request', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_uu.reset_complete', 'reset_complete', $options = array('control' => 'jform'));
		if (empty($form)) {
			return false;
		}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_uu.reset_confirm', 'reset_confirm', $options = array('control' => 'jform'));
		if (empty($form)) {
			return false;
		}
	...
	function processResetComplete($data)
	{
		...
		$token	= $app->getUserState('com_uu.reset.token', null);
		$userId	= $app->getUserState('com_uu.reset.user', null);
		...
		$app->setUserState('com_uu.reset.token', null);
		$app->setUserState('com_uu.reset.user', null);
	...
	function processResetConfirm($data)
	{
		...
		$app = JFactory::getApplication();
		$app->setUserState('com_uu.reset.token', $crypt.':'.$salt);
		$app->setUserState('com_uu.reset.user', $user->id);
	...
	public function processResetRequest($data)
	{
		...
		$itemid = UuHelperRoute::getLoginRoute();
		$itemid = $itemid !== null ? '&Itemid='.$itemid : '';
		$link = 'index.php?option=com_uu&view=reset&layout=confirm'.$itemid;
...
//		$data['link_html']	= JRoute::_($link, true, $mode);
        $data['reset_link_text']	= JRoute::_($link, false, $mode);
        $data['reset_link_html']	= JRoute::_($link, true, $mode);
		$data['token']		= $token;
        ...
        $subject = $mailhelper->renderMail($conf->get('reset_mail_subject'),$data,$user);

        //COM_USERS_EMAIL_PASSWORD_RESET_BODY
        $body = $mailhelper->renderMail($conf->get('reset_mail_body'),$data,$user);

		// Send the password reset request email.
		$return = JFactory::getMailer()->sendMail($data['mailfrom'], $data['fromname'], $user->email, $subject, $body,$htmlmode);
		// Check for an error.
		if ($return !== true) {
